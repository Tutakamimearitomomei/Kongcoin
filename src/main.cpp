...
#if defined(NDEBUG)
# error "Kongcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x011e1d0d28c815d736bef7e5d8dd138d29282e512edc9ae6d0d68ce21f79c047");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kongcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kongcoin Signed Message:\n";
...
{
    // Kongcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kongcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kongcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1492644006, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = " New york times 20 Apr 2017 Zuckerberg Sees Augmented in Facebook";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 4000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0492832ceee82e485d910fb16ea30b2c21dbb4b0b779d758ee894a061f5c6430174f2314a052dd217deebc88431b69052fbdc393d1aa107a94b3827a4fb28d1e12") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1492644006;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1147754;
        ...
        assert(block.hashMerkleRoot == uint256("0x283026fe66aaadd199d8e41fcbec8ec8fade712d3f3d6ed86794a5fed5a8c5df"));
...
unsigned char pchMessageStart[4] = { 0xb3, 0xe7, 0xd8, 0x1f }; // Kongcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KongcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KongcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KongcoinMiner(CWallet *pwallet)
{
    printf("KongcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kongcoin-miner");
    ...
    try { loop {
        ...
        printf("Running KongcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KongcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KongcoinMiner, pwallet));
}
